  ...
  return `
    <div class="web-result-tile ${selected ? 'selected' : ''}" id="webTile${index}">
      ...
      <div class="web-result-body">
        ...
        ${r.chef || r.source ? `<div class="web-result-source">${r.chef ? `<span class="web-result-chef">${esc(r.chef)}</span>` : ''}${r.chef && r.source ? ' · ' : ''}${r.source ? `<span>${esc(r.source)}</span>` : ''}</div>` : ''}
        ${r.source_attribution && !r.chef && !r.source ? `<div class="web-result-source">${esc(r.source_attribution)}</div>` : ''}
